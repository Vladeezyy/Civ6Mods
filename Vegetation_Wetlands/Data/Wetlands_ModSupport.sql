...
-- Boom's Pantheon Rebalance
--------------------------------------------------------------
DELETE FROM RequirementSetRequirements WHERE RequirementSetId='PLOT_HAS_REEDS_REQUIREMENTS' AND RequirementId='REQUIRES_PLOT_HAS_FLOODPLAINS_GRASSLAND';
DELETE FROM RequirementSetRequirements WHERE RequirementSetId='PLOT_HAS_REEDS_REQUIREMENTS' AND RequirementId='REQUIRES_PLOT_HAS_FLOODPLAINS_PLAINS';
--------------------------------------------------------------
...
WHERE	ID='Pok_Amun_Ra_Grass_Floodplains_Faith';

INSERT INTO Adjacency_YieldChanges
		(ID,									Description,											YieldType,	YieldChange,	TilesRequired,	AdjacentFeature)
SELECT	'Pok_Amun_Ra_JNR_Swamp_Faith',			'LOC_POK_DISTRICT_AMUN_RA_JMR_SWAMP_FAITH',				YieldType,	YieldChange,	TilesRequired,	'FEATURE_JNR_SWAMP'
FROM	Adjacency_YieldChanges
WHERE	ID='Pok_Amun_Ra_Grass_Floodplains_Faith';
...
WHERE	YieldChangeId='Pok_Amun_Ra_Grass_Floodplains_Faith';

INSERT INTO District_Adjacencies
		(DistrictType,	YieldChangeId)
SELECT	DistrictType,	'Pok_Amun_Ra_JNR_Swamp_Faith'
FROM	District_Adjacencies
WHERE	YieldChangeId='Pok_Amun_Ra_Grass_Floodplains_Faith';
--------------------------------------------------------------
