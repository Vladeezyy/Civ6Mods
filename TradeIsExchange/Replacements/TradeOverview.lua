...
local TINT_TRADER_FADED_ARROW			= UI.GetColorValue(0.25, 0.55, 0.55, 1);
...
local TINT_OTHER_TRADER_CITY_PLOT		= UI.GetColorValue(0.75, 1, 1, 0.75)
local TINT_OTHER_TRADER_CITY_SHADOW		= UI.GetColorValue(0, 1, 1, 1)
local TINT_OTHER_TRADER_CITY_BORDER		= UI.GetColorValue(0, 1, 1, 0.75)
-- Tunnel Plots Colors
...
function AddRouteInstanceFromRouteInfo( routeInfo:table )
	...
	-- Add number of Cities on the path
    ...
    elseif m_currentTab == TRADE_TABS.ROUTES_TO_CITIES then
        ...
        tooltipString = (   Locale.Lookup("LOC_TRADE_TURNS_REMAINING_HELP_TOOLTIP") .. "[NEWLINE]" ..
                            Locale.Lookup("LOC_TRADE_TURNS_REMAINING_TOOLTIP_BREAKER") .. "[NEWLINE]" ..
                            Locale.Lookup("LOC_TRADE_TURNS_REMAINING_ROUTE_LENGTH_TOOLTIP", tradePathLength, nCities) );
    ...
    elseif m_currentTab == TRADE_TABS.AVAILABLE_ROUTES and m_AvailableTraders ~= nil and table.count(m_AvailableTraders) > 0 then
        ...
        if m_AvailableTraders[routeInfo.OriginCityID] ~= nil and table.count(m_AvailableTraders[routeInfo.OriginCityID]) > 0 then
            ...
            routeInstance.GridButton:RegisterCallback( Mouse.eLClick,
                function()
                    SelectFreeTrader(tradeUnit, routeInfo.OriginCityPlayer, routeInfo.OriginCityID,  routeInfo.DestinationCityPlayer, routeInfo.DestinationCityID);
                end
            );
...
function ClearForeignTraderLens()
	UI.DeselectAllUnits();
...
function ShowForeignTraderPath(playerID:number, unitID:number)
    ...
    for _, originCity in originCities:Members() do
        ...
        for _, routeInfo in ipairs(outgoingRoutes) do
            if unitID == routeInfo.TraderUnitID then
				...
				KpathPlots = tradeManager:GetTradeRoutePath(originCity:GetOwner(), originCity:GetID(), destinationCity:GetOwner(), destinationCity:GetID());
				local cityPlots:table = {};		
				local tunnelPlots:table = {};
				local pathSegment:table = {};		
				
				local kVariations:table = {};
                local lastElement:number = table.count(KpathPlots);
                ...
                -- Turns out the mod missed portals
				for i,plot in pairs(pathPlots) do
					...
					if pPlot:IsCity() or pEntrance >= 0 or pExit >= 0 then
						...
						--						
						UILens.SetLayerHexesPath( m_TradeRouteLens, Game.GetLocalPlayer(), pathSegment, { }, TINT_TRADER_UNOWNED_ARROW );
						pathSegment = {}
						if pExit >= 0 then table.insert(pathSegment, pExit)	end
						table.insert(pathSegment, plot)
						if pEntrance >= 0 then table.insert(pathSegment, pEntrance)	end
				...
				--============================================================================================				
				-- Color Setting
				m_TradeRouteHexPath:SetBorderColors(TRADE_PATH_CHANNEL, TINT_OTHER_TRADER_PATH_SHADOW, TINT_OTHER_TRADER_PATH_BORDER)
				m_TradeRouteHexPath:SetHighlightColor(TRADE_PATH_CHANNEL, TINT_OTHER_TRADER_PLOT)
				
				m_TradeRouteCities:SetBorderColors(TRADE_PATH_CITY_CHANNEL, TINT_OTHER_TRADER_CITY_SHADOW, TINT_OTHER_TRADER_CITY_BORDER)
				m_TradeRouteCities:SetHighlightColor(TRADE_PATH_CITY_CHANNEL, TINT_OTHER_TRADER_CITY_PLOT)
                ...
                --UILens.SetLayerHexesPath( m_TradeRouteLens, Game.GetLocalPlayer(), { }, kVariations, DEFAULT_TINT);
            end
        end
    end
end
...
function SelectUnit( unit:table )
	...
	if m_currentTab == TRADE_TABS.AVAILABLE_ROUTES then return end
	...
	if localPlayer ~= -1 and localPlayer ~= unit:GetOwner() then
        UI.DeselectAllUnits();
        UI.DeselectAllCities();
        -- [here we try the thing]
		local unitID = unit:GetID();
		local owner = unit:GetOwner()
		ShowForeignTraderPath(owner, unitID)
		UI.LookAtPlotScreenPosition( unit:GetX(), unit:GetY(), 0.42, 0.5 );
		--
    end
	--
...
function SelectFreeTrader( unit:table, originCityOwnerID:number, originCityID:number, destinationCityOwnerID:number, destinationCityID:number )
    local localPlayer = Game.GetLocalPlayer();
