...
INSERT OR IGNORE INTO Modifiers
		(ModifierId,								ModifierType)
SELECT	'JNR_HANGAR_TRAIN_AIRATTACK_PRODUCTION',	'MODIFIER_JNR_SINGLE_CITY_ADJUST_MILITARY_UNITS_PRODUCTION'
FROM	UnitPromotionClasses
...
INSERT OR IGNORE INTO ModifierArguments
		(ModifierId,								Name,				Value)
SELECT	'JNR_HANGAR_TRAIN_AIRATTACK_PRODUCTION',	'Amount',			50
FROM	UnitPromotionClasses
WHERE	PromotionClassType='PROMOTION_CLASS_AIR_ATTACK';
...
INSERT OR IGNORE INTO ModifierArguments
		(ModifierId,								Name,				Value)
SELECT	'JNR_HANGAR_TRAIN_AIRATTACK_PRODUCTION',	'PromotionClass',	PromotionClassType
FROM	UnitPromotionClasses
WHERE	PromotionClassType='PROMOTION_CLASS_AIR_ATTACK';
--------------------------------------------------------------
...
INSERT OR IGNORE INTO BuildingModifiers
		(ModifierId,								BuildingType)
SELECT	'JNR_HANGAR_TRAIN_AIRATTACK_PRODUCTION',	'BUILDING_HANGAR'
FROM	BuildingModifiers
WHERE	BuildingType='BUILDING_HANGAR'
		AND ModifierId='JNR_HANGAR_TRAIN_FIGHTER_PRODUCTION'
		AND EXISTS (SELECT * FROM FROM	UnitPromotionClasses WHERE PromotionClassType='PROMOTION_CLASS_AIR_ATTACK');
--------------------------------------------------------------